...
**Last committed version:** v0.13.2 · **Current status:** P7d-1 is locally complete and committed; P0 remote CI/deployed-shell gates remain · **Last verified:** 2026-07-25 (S28; timestamps are owner-directed)
...
| Fact | Verified state |
|---|---|
| Version | The local P0 series is **v0.1.0…v0.1.7**, P1 is **v0.2.0…v0.2.4**, P2 is **v0.3.0…v0.3.2**, P3 is **v0.4.0…v0.4.5**, P4 is **v0.5.0…v0.5.3**, P5 is **v0.6.0…v0.6.4**, P6a is **v0.7.0…v0.7.3**, P7a is **v0.8.0…v0.8.3**, and P7b is **v0.8.4…v0.8.6**. The latest commit is `Commit v0.8.6: Van Westendorp Acceptance + P7b Handoff /3-F5-T6`, post-dated 2026-07-25 00:45 +05:30. `package.json`, both lockfile root versions, the footer, and README's Version Control block agree at v0.8.6. P0 remains open despite its versioned local history. |
| Git | Branch `main` tracks `origin/main` and is **forty-two local commits ahead** after S23. `origin/main` remains at `da5ba98` (v0.0.4); no push was attempted. There are no tags, other local branches, or submodules. |
| Working tree | P5’s committed design editor, pure linter, scenario-to-engine adapter, test coverage, browser flow, and status updates remain intact. P6a’s live Simulate surface, sweep adapter, acceptance tests, and v0.7.3 metadata are committed. P7a’s pure uncertainty engine, state adapter, Analyze view, acceptance coverage, and v0.8.3 metadata are committed. P7b’s pure PSM engine, typed durable study records, Analyze — Research workbench, demo CSV, acceptance coverage, and v0.8.6 metadata are committed; generated build/test output remains ignored. |
| Product | P1–P2's tested economics core and P3 scenario foundation remain intact. P4 adds a first-run template picker, model segment cards, P50-centred size/WTP bands, explicit within-segment spread, provenance selectors, a keyboard-navigable allocation/direct-value matrix, glossary popovers, and engine-backed preview KPIs. P5 adds editable tiers, free tier, feature fences, add-ons, design alternatives, and a deterministic, cited linter with engine-backed counterfactuals. P6a reveals live KPIs, buyer sorting, a reconciled value waterfall, and deferred 400-point per-tier price sweeps with equivalent data tables and a polite live announcement. P7a adds deterministic P10/P50/P90 MRR bands, common-random-number design comparisons, one-at-a-time tornado drivers, and provenance-led validation to-dos. P7b adds fielded Van Westendorp PSM input, transparent exclusion of price-order violators, four cumulative curves, piecewise-linear PMC/PME/IPP/OPP markers, and an explicitly non-evidentiary illustrative mode. Exact own/competitor identity ties remain competitor-first. |
...
The product thesis, architecture, economic contracts, release cut, and phase gates are documented, audited fresh-eyes (S4), independently cross-checked (S5), and **owner-finalized for the v1 build (S6)**. Four owner-authorized planning commits exist: initialization (`v0.0.1`), contract corrections (`v0.0.2`), the Wind Tunnel rename (`v0.0.3`), and the pre-build check (`v0.0.4`); eight post-dated local P0 commits (`v0.1.0…v0.1.7`) provide the executable scaffold. S9–S12 delivered the committed P1/P2 economics core; S13–S14 delivered P3's scenario foundation; S15–S16 delivered the committed P4 Model surface; S17–S18 delivered the P5 Design surface and linter; S19–S20 delivered the committed P6a live Simulate reveal; S21 delivered the committed P7a uncertainty workbench; and S22–S23 delivered and committed P7b’s Van Westendorp workbench with 70 unit tests, six Chromium flows, zero serious/critical axe findings on the new flow, and a 281.9 KiB gzip bundle. P0 is locally verified but not complete because GitHub Actions has not run on a test PR and Vercel is not connected for the deployed-shell check.
...
3. **Next implementation phase:** begin **P8 — Pricing Decision Record** (§6), carrying the active design, simulation, uncertainty, PSM research, and linter findings into one traceable Markdown/print artifact.
4. **Future pushes:** the local P0/P1/P2/P3/P4/P5/P6a/P7a/P7b series is committed but unpushed. Push only when separately asked.
...
P5 is locally complete, committed, and verified. Its persisted schema intentionally stays distinct from engine contracts, and its share payload excludes research artifacts by construction. Design edits preserve schema validation and scenario undo/autosave; the linter receives only an injected, deterministic menu re-simulation path. P6a is locally complete, committed, and verified: the Simulate tab consumes the same state-to-engine adapter as the linter, then renders live outcomes and exposes matching tables rather than maintaining a second calculation path. P7a is locally complete, committed, and verified: its Monte Carlo path samples only size/WTP confidence bands, holds within-segment σ and price/menu assumptions fixed, shares each indexed draw vector across all designs, and keeps the tornado as a one-at-a-time active-design sensitivity readout. P7b is locally complete, committed, and verified: it accepts fielded manual/CSV PSM responses, persists the original records with their source, excludes rather than changes monotonicity violators, uses the §4.7 curve and interpolation convention, and labels the deterministic model-derived teaching dataset “simulated — not evidence.”
