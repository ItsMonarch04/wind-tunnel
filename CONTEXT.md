...
**Last committed version:** v0.2.4 · **Current status:** P1 locally complete; P0 remote CI/deployed-shell gates remain · **Last verified:** 2026-07-16 (S10)
...
| Fact | Verified state |
|---|---|
| Version | The local P0 series is **v0.1.0…v0.1.7** and the P1 series is **v0.2.0…v0.2.4**; this handoff lands in `Commit v0.2.4: CI Coverage + P1 Handoff /5-F5-T6`, post-dated 2026-07-20 01:10 +05:30. `package.json`, both lockfile root versions, the footer, and README's Version Control block agree. P0 remains open despite its versioned local history. |
| Git | Branch `main` tracks `origin/main` and is **thirteen local commits ahead** after S10. `origin/main` remains at `da5ba98` (v0.0.4); no push was attempted. There are no tags, other local branches, or submodules. |
| Working tree | Clean after the P1 series: pure engine modules and contract tests, `docs/MODEL-SPEC.md`, CI spec-coverage reporting, and the P1 handoff are committed. Generated build/test output remains ignored. |
| Product | The static shell remains unchanged. P1 now has a tested pure core for lognormal statistics, joint tier/add-on/competitor offer expansion, and upper-envelope selection. Economics, scenarios, persistence, and product surfaces beyond the shell remain unimplemented. Exact own/competitor identity ties are explicitly competitor-first, so their full choice mass is recorded as competitor loss rather than own revenue. |
...
3. **Next implementation phase:** begin **P2 — economics readouts** (§6) from the committed P1 core. Reuse its offer expansion/envelope path; do not duplicate competitor attribution. P2 must implement §4.3–§4.5 and their named tests.
4. **Future pushes:** the local P0 series is committed but unpushed. Push only when separately asked.
...
P1's implemented core is intentionally limited to the pure primitives and their tests. Its local verification is green, including the P0 privacy/bundle/browser gates; economics, scenario, and persistence code remain unimplemented and cannot yet be assessed.
